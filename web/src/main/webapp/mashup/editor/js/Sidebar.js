...
Sidebar.prototype.init = function()
{
	//var dir = STENCIL_PATH;
        ...
        var keys =  digitnexus.editor.me.mashupConfigManager.getConfigNodesKeySet();
        if(keys == null || keys.size < 1) {
            return "No key can be found!";
        }    
        //this.addDefaultPalette();
        var size = keys.size();
        for(var index = 0; index < size; index++) {
           ...
           this.addPaletteByKey(false,key);
        }
...
Sidebar.prototype.addDefaultPalette = function()
{
	this.addPalette(digitnexus.editor.Constants.ED_PALLETE_DEFAULT_GROUP, 
        mxResources.get(digitnexus.editor.Constants.ED_PALLETE_DEFAULT_GROUP), 
        true, mxUtils.bind(this, function(content)
	{    
	    //content.appendChild(this.createVertexTemplate('swimlane', 200, 200, 'Container'));
            //'line;direction=south', 10, 160 
            // content.appendChild(this.createEdgeTemplate('edgeStyle=none', 100, 100));

    	    content.appendChild(this.createEdgeTemplate('edgeStyle=none;endArrow=none;', 100, 100, null));
	    content.appendChild(this.createEdgeTemplate('edgeStyle=none', 100, 100));
//	    content.appendChild(this.createEdgeTemplate('edgeStyle=elbowEdgeStyle;elbow=horizontal', 100, 100, null));
//	    content.appendChild(this.createEdgeTemplate('edgeStyle=elbowEdgeStyle;elbow=vertical', 100, 100, null));
//	    content.appendChild(this.createEdgeTemplate('edgeStyle=entityRelationEdgeStyle', 100, 100, null));
//	    content.appendChild(this.createEdgeTemplate('edgeStyle=segmentEdgeStyle', 100, 100, null));
//	    content.appendChild(this.createEdgeTemplate('edgeStyle=orthogonalEdgeStyle', 100, 100, null));
//	    content.appendChild(this.createEdgeTemplate('shape=link', 100, 100, null));
//	    content.appendChild(this.createEdgeTemplate('arrow', 100, 100, null));
	}));
};

Sidebar.prototype.addPaletteByKey = function(expand,key)
{
        var label = mxResources.get(key,null,key);
	this.addPalette(key,label , expand, mxUtils.bind(this, function(content)
	{
	    //content.appendChild(this.createVertexTemplate('swimlane', 200, 200, 'Container'));
            //'line;direction=south', 10, 160          
            var nodeSet = digitnexus.editor.me.mashupConfigManager.getConfigNodes(key);             
            if(null == nodeSet || nodeSet.size() < 1) {
                return "No node can be found!";
            }    
            var ul = document.createElement('ul');
            ul.className='myUl';
            content.appendChild(ul);
            var size = nodeSet.size();
            var ops = [];
            var bos = [];
            for(var index = 0; index < size; index++) {
                var node  = nodeSet.get(index);
                if(null == node) {
                    continue;
                }          
                if(node.isOperator) {
            ...
            var func = mxUtils.bind(this, function(node){
            	 var nameWidth = digitnexus.utils.getStringWidthAsPix(node.name)+45;
                 var nameHeight = node.editorProperties.get(digitnexus.editor.Constants.ED_ELEMENT_HEIGHT);
                 var style = node.editorProperties.get(digitnexus.editor.Constants.ED_NODE_STYLE);
                 if(null == style) {
                     style = digitnexus.editor.Constants.ED_NODE_DEFAULT_STYPE;
                 }
                 ul.appendChild(this.createVertexTemplate(style, nameWidth, nameHeight, node));
            });
...
Sidebar.prototype.addGeneralPalette = function(expand)
{
	...
	{
	    //content.appendChild(this.createVertexTemplate('swimlane', 200, 200, 'Container'));
            var nodes =  digitnexus.editor.me.getConfigNodes();
            if(nodes == null) {
                return "No node can be found!";
            }
            ...
            for(var index = 0; index < size; index++) {
                ...
                var nodeSet = nodes.get(key);              
                if(null == nodeSet) {
                    continue;
                }              
                var nameWidth = digitnexus.utils.getStringWidth(nodeSet.name)+45;
                var nameHeight = nodeSet.editorProperties.get(digitnexus.editor.Constants.ED_ELEMENT_HEIGHT);
                var style = nodeSet.editorProperties.get(digitnexus.editor.Constants.ED_NODE_DEFAULT_STYPE);
                if(null == style) {
                    style = digitnexus.editor.Constants.ED_NODE_DEFAULT_STYPE;
                }
                ul.appendChild(this.createVertexTemplate(style, nameWidth, nameHeight, nodeSet));
            }
            /**
 ...
 */
Sidebar.prototype.createItem = function(cells, value)
{
       var icon = null;
        var elt = null;
        if(value == null) {
            ...
            /**
             *<div style="background:url('img.gif') no-repeat;width:100px;height:50px">����Ϳ���ͼƬ�ϼ�������</div>
             */
            var icon =  value.editorProperties.get(digitnexus.editor.Constants.ED_ELEMENT_ICON);
            if(null == icon) {
                icon = 'checkmark.gif';
            }
            ...
            //elt.style = "no-repeat";
            //elt.style.background = 'url(' + icon + ')';
            //elt.setAttribute('href', 'javascript:void(0);');
            elt.className = 'myGetItem';
            //elt.src = icon;
            //mxUtils.write(elt, value == null? "": value.name);
            // Blocks default click action
            mxEvent.addListener(elt, 'click', function(evt)
            {
                    mxEvent.consume(evt);
	...
	return function(graph, evt, target, x, y)
	{
		...
		if (cells.length > 0)
		{
                    var cs = [];
                        if(cells.length == 1) {
                             if(cells[0].isVertex()) {  
                                if(null == cells[0].value) {
                                     	console.log('fail to create node for cell id: ' + cells[0].getId());
                                        return;
                                     }
                                    var newValue = digitnexus.editor.me.mashupInfoManager.createNode(cells[0].value);
                                    var cacheValue = cells[0].value;
                                    cells[0].value = null;
                                    var newCell = mxUtils.clone(cells[0], null, true);
                                    cells[0].value = cacheValue;
                                    newCell.value = newValue;    
                                    newCell.nodeId = newValue.nodeId;
                                    cs.push(newCell);
                                    //digitnexus.editor.me.addEditableLisneter(n) 
                            }else {
                                cs.push(cells[0]);
                            }
                      } else {
                        ...
                        //cells = cs;
			var validDropTarget = (target != null) ? graph.isValidDropTarget(target, cs, evt) : false;
			var select = null;
			...
			if (allowSplit && graph.isSplitEnabled() && graph.isSplitTarget(target, cs, evt))
			{
				graph.splitEdge(target, cs, null, x, y);
				select = cs;
			}
			else if (cs.length > 0)
			{
				//select = graph.moveCells(cs, x, y, true, target);        
				select = graph.importCells(cs, x, y, target);
				/*graph.stopEditing(false);
				for(var vertex in cs) {
					cs[vertex].geometry.x = x;
					cs[vertex].geometry.y = y;
					graph.addCell(cs[vertex]);
					graph.setSelectionCell(cs[vertex]);
				}*/
				if (select != null && select.length > 0){
                                    digitnexus.editor.me.addOverlay(select[0]);
                                }
			}
...
Sidebar.prototype.createDragSource = function(elt, dropHandler, preview)
{
	...
	dragSource.getDropTarget = function(graph, x, y)
	{
		...
		{
			console.log('drop target is invalid');
			target = null;
		}
...
Sidebar.prototype.createVertexTemplate = function(style, width, height, value)
{
	...
	return this.createVertexTemplateFromCells(cells, width, height,value);
};

/**
 * Creates a drop handler for inserting the given cells.
 */
Sidebar.prototype.createVertexTemplateFromCells = function(cells, width, height,value)
{
	var elt = this.createItem(cells,value);
