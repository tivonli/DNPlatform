...
Dialog.prototype.close = function()
{
    ...
    if(this.container != null &&  this.bg.parentNode != null ) {
...
function AboutDialog(editorUi)
{
    ...
    mxUtils.write(h3, mxResources.get('about') + ' Mashup Editor');
    div.appendChild(h3);
    var img = document.createElement('img');
    img.style.border = '0px';
    img.setAttribute('width', '176');
    img.setAttribute('width', '151');
    img.setAttribute('src', IMAGE_PATH + '/logo1.png');
    div.appendChild(img);
    mxUtils.br(div);
    mxUtils.write(div, 'Mash-up Editor for report and dashboard ' + digitnexus.editor.Constants.VERSION);
    mxUtils.br(div);
    var link = document.createElement('a');
    link.setAttribute('href', 'http://www.digitnexus.com/');
    link.setAttribute('target', '_blank');
    mxUtils.write(link, 'www.digitnexus.com');
 ...
 * 
 */
function CreateMashupDefinitionDialog(okCallback,cancelCallback)
{
    var table = new digitnexus.utils.Table();
    digitnexus.editor.me.mashupInfoManager.createMashupDefinitionContent(table);
    var tbody = table._tbody;
    var row = document.createElement('tr');
    var submit = mxResources.get('ok',null,'ok');
    var td = document.createElement('td');
    td.style.fontSize = '10pt';
    td.style.align = 'right';
    td.appendChild(mxUtils.button(submit, mxUtils.bind(this,function()
    {
        if(okCallback != null) {
            okCallback(table);
        }
    ...
    td.style.fontSize = '10pt';
    td.style.align = 'right';  
    var cancel = mxResources.get('cancel',null,'cancel');
    td.appendChild(mxUtils.button(cancel, mxUtils.bind(this, function()
    {
        if(cancelCallback != null) {
            cancelCallback(table);
        }
    }))); 
    row.appendChild(td);
    tbody.appendChild(row);
...
function PreferencesDialog(editorUi)
{
    this.pages = new digitnexus.utils.HashMap();
    this.editorUi = editorUi;
    var currentSet = null;
    var mainPanel = editorUi.createDiv('mainPanelStyle')
        
    var moduleList = editorUi.createDiv('moduleListStyle')
    mainPanel.appendChild(moduleList);

    var preferenceDiv = editorUi.createDiv('preferencePageStyle')
    this.preferencePage = new digitnexus.utils.Table('preferenceTablePage');
        
    preferenceDiv.appendChild(this.preferencePage.table);
    mainPanel.appendChild(preferenceDiv);
        
    var controlPanel = editorUi.createDiv('controlPanelStyle')
    mainPanel.appendChild(controlPanel);
        
    var listTree = new digitnexus.utils.ListView(moduleList,mxUtils.bind(this, function(moduleName) {
        if(null == moduleName) {
            return;
        }
        var key = moduleName.srcElement.id;
        if(null == key || '' == key.trim() || this.currentSet == key) {
            return;
        }
        this.combineValueToPage();
        var inputlist =  this.preferencePage.inputlist;
        if(inputlist != null && inputlist.size() > 0) {
            inputlist.removeAll();
        }
        this.preferencePage.removeAllRows();
        this.clickModule(key);
    }));
        
    var Value = function(key,valueSet){
        this.key = key;
    ...
    var listAdapter = new function(model) {
        this.model = model;
        this.getList = function() {
            var keySet = this.model.settings.keySet();
            if(keySet == null) {
                return null;
            }
            var arr = [];
            ...
            for(var index = 0 ; index < size; index++) {
                ...
                var valueSet = this.model.settings.get(key);
                arr.push(new Value(key,valueSet));
            }
            return arr;
        }
    }(digitnexus.editor.me.preferenceManager.userProfile);
        
    listTree.setModel(listAdapter);
        
    controlPanel.appendChild(mxUtils.button(mxResources.get('ok', null, 'ok'), mxUtils.bind(this,function() {
        this.combineValueToPage();
        var keyset = this.pages.keySet();
        if(keyset == null) {
            return;
        }
        var size = keyset.size();
        for(var index = 0 ; index < size; index++) {
            ...
            var rows = this.pages.get(key);
            //var settings = digitnexus.editor.me.userProfile.getModelList(key);
            ...
            for(var i = 0 ;i <rowNum; i++) {
                ...
                digitnexus.editor.me.preferenceManager.userProfile.put(key,row);
            }
        } 
        this.destroy();
    ...
    controlPanel.appendChild(mxUtils.button( mxResources.get('restoreDefault', null, 'restoreDefault'), mxUtils.bind(this,function() {
        if(this.currentSet == null) {
            return;
        }
        this.pages.remove(this.currentSet);
        var settings = digitnexus.editor.me.preferenceManager.userProfile.getModelList(this.currentSet);
        var size = settings.size();
        for(var i = 0 ;i <size; i++) {
            var nv = settings.get(i);
            if(null == nv) {
                continue;
            }
            var cloneNv = mxUtils.clone(nv);
            cloneNv.value = nv.defaultValue;
            digitnexus.editor.me.preferenceManager.userProfile.put(this.currentSet,cloneNv);
        }
        var inputlist =  this.preferencePage.inputlist;
        if(inputlist != null && inputlist.size() > 0) {
            inputlist.removeAll();
        ...
        this.preferencePage.removeAllRows();
        this.clickModule(this.currentSet);
    }), null));
    controlPanel.appendChild(mxUtils.button( mxResources.get('cancel', null, 'cancel'), mxUtils.bind(this,function() {       
        this.destroy();
        this.editorUi.hideDialog();
    }), null));
    ...
    var nl = digitnexus.editor.me.preferenceManager.userProfile.getModelNameList();
    if(nl != null && nl.size() > 0) {
        this.clickModule(nl.get(0));
    }
};

PreferencesDialog.prototype.clickModule = function(moduleName) {
    var values = this.pages.get(moduleName);
    if(null == values) {
        values = digitnexus.editor.me.preferenceManager.userProfile.getModelList(moduleName);
    }
    if(null == values) {
        return;
    }
    ...
    this.currentSet = moduleName;
    for(var index = 0 ; index < size; index++) {
        var value = values.get(index);
        if(null == value) {
            continue;
        }
        this._addOneValue(value)
    }
}
         
PreferencesDialog.prototype._addOneValue= function(value) {
    ...
    switch(uiType) {
        case 'text':
            input = digitnexus.utils.formFactory.createTextbox(value.name,value.value);
            break;
        case 'password':
            input = digitnexus.utils.formFactory.createPassword(value.name,value.value);
            break;
        case 'hidden':
            input = digitnexus.utils.formFactory.createHidden(value.name,value.value);
            break;
        case 'file':
            input = digitnexus.utils.formFactory.createFile(value.name,value.value);
            break;
        case 'submit':
            input = digitnexus.utils.formFactory.createSubmit(value.name,value.value);
            break;
        case 'reset':
            input = digitnexus.utils.formFactory.createReset(value.name,value.value);
            break;
        case 'radio':
            input = digitnexus.utils.formFactory.createRadiobox (value.name,value.availableValue,value.value);
            break;
        case 'check':
            input = digitnexus.utils.formFactory.createCheckbox(value.name,value.availableValue,value.value);
            break;
        case 'textarea':
            input = digitnexus.utils.formFactory.createTextarea(value.name,value.value);
            break;
        case 'combobox':
            input = digitnexus.utils.formFactory.createCombobox(value.name,value.value);
        default:
            console.log("UI type Error: "  + uiType);
            input = digitnexus.utils.formFactory.createTextbox(value.name,value.value);
    }
    if(null == input) {
        console.log("Fail to create input type: "  + uiType);
        return;
    }
    if(value.readOnly) {
        input.readOnly = true;
    ...
    var row = this.preferencePage.addRow('preferencePageRow');
    this.preferencePage.addTd(mxResources.get(value.name, null,value.name));
    this.preferencePage.addTd(input);
    return row;
}
PreferencesDialog.prototype._getNameValue = function(settings,name) {
    if(null == settings || null == name || name.trim() == '') {
        return null;
    }
    var size = settings.size();
    if(size < 1) {
...
PreferencesDialog.prototype.combineValueToPage = function() {
    if(this.preferencePage == null) {
        console.log( 'preference page is NULL');
        return ;
    }
    if(this.currentSet == null) {
        return ;        
    ...
    var keyset = inputlist.keySet();
    var size = keyset.size();
    if(size < 1) {
        return;
    }
    var inupts =  this.pages.get(this.currentSet);
    if(inupts == null) {
        inupts = new digitnexus.utils.Set(); 
    } 
    var settings = digitnexus.editor.me.preferenceManager.userProfile.getModelList(this.currentSet);
    for(var index =0; index < size; index++) {
        var input = inputlist.get(keyset.get(index));
        if(null == input) {
            continue;
        }
        var value =  digitnexus.utils.formFactory.getNodeValue(input);
        var name = input.name;
        var nv = this._getNameValue(settings,name);
        if(null == nv) {
            continue;
        }
        var cloneNv = mxUtils.clone(nv);
    ...
    this.pages.put(this.currentSet,inupts);    
}
       
function BeforeUploadDialog(editorUi)
{
    this.editorUi = editorUi;
    this.isClosed = false;
    var closeDialog = mxUtils.bind(this,function() {   
        if( this.isClosed) {
            return;
        }
        this.isClosed = true;
    ...
    var table  = new digitnexus.utils.Table('beforeUploadDialog');
    table.addRow();
    var td = table.addTd();
    var button = mxUtils.button(mxResources.get('saveAndExit', null, 'Save And Exit'),mxUtils.bind(this,function() {
        digitnexus.editor.me.saveMashupDefinition();
        digitnexus.editor.me.preferenceManager.saveUserProfile();
        closeDialog();
    }));
    td.appendChild(button);
        
    button = mxUtils.button(mxResources.get('notSaveAndExit', null, 'Not Save And Exit'),mxUtils.bind(this,function() {
        closeDialog();
    }));
    td.appendChild(button);
        
    var button = mxUtils.button(mxResources.get('cancel', null, 'Cancel'),mxUtils.bind(this,function() {
        closeDialog();
        this.result = '';
    }));
    td.appendChild(button);
